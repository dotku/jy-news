...
export function ArticleViews({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    fetch(`/api/articles/${encodeURIComponent(slug)}/stats`)
      .then((r) => r.json())
...
export function ArticleLikeButton({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    const likedSlugs = JSON.parse(localStorage.getItem("liked_articles") || "[]");
  ...
  const handleLike = async () => {
    ...
    localStorage.setItem("liked_articles", JSON.stringify(likedSlugs));
  };
    ...
    <button
      ...
      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm font-medium transition-all ${
        liked
          ...
          : "border-zinc-200 text-zinc-500 hover:border-red-300 hover:bg-red-50 hover:text-red-600 dark:border-zinc-700 dark:text-zinc-400 dark:hover:border-red-800 dark:hover:bg-red-950 dark:hover:text-red-400"
      }`}
    >
      <svg
        className="h-4 w-4"
